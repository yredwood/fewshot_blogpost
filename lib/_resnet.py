...
class ResNet_CIFAR(object):
    def __init__(
            self, name, depth,
            gpu_idx=0, reuse=None):

        self.depth = depth
        self.pre_params = [] 
        self.post_params = [] # will not be used
...
#                    Dense(64, n_classes, name='dense_logit'))
    ...
    def __call__(self, x, train):
        def _apply_block(x, downsample, train, n_b):
            _rs = x # shortcut
        ...
        for n_group, n_block in enumerate(self.n_blocks):
            n_in = self.n_filters[n_group]
            n_out = self.n_filters[n_group+1]
            downsample = 0 if n_in == n_out else 1
            for bn in range(n_block):
                x = _apply_block(x, downsample, train, block_cnt)
