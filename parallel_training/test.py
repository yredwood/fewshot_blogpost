...
#        args.epoch_list = '0.7'
    ...
    for i in range(num_parallel):
        nets[i] = ConvNet(args.model_name+'_{}'.format(i),
                isTr=False, reuse=False, config=config, arch=args.arch, nway=len(trg_classes[i]),
                input_dict=input_dict)
        feats[i] = nets[i].outputs['pred']
        tnvars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                scope=args.model_name+'_{}/*'.format(i))
