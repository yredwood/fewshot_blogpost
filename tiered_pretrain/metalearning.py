...
def parse_args():
    ...
    parser.add_argument('--batch_size', dest='batch_size', default=0, type=int)
...
#        args.epoch_list = '0.7'
    ...
    train_dataset = TieredImageNet('tiered', 'images/train', class_config=trg_class)
    tr_x, tr_y = train_dataset.input_fn(args.batch_size, train_mode=True)
    train_input_dict = {'x': tr_x, 'y': tr_y, 'lr': tf.placeholder(tf.float32)}

    test_dataset = TieredImageNet('tiered', 'images/train', class_config=trg_class)
    ...
    train_net = Net(args.model_name+'_{}'.format(args.parall_n),
            ...
            arch=args.arch, nway=nway, input_dict=train_input_dict)
    ...
    if args.train:
        ...
        max_iter = nway*kshot*10//args.batch_size
        ...
        for i in range(1, max_iter+1):
            ...
            fd = {train_net.inputs['lr']:args.lr}
